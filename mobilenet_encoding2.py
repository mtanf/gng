...
def get_mobilenet_classifier(input_shape, trainable_base = False, lr = 1e-3):
    ...
    if trainable_base:
        ...
        for i in range(len(base_model.layers)):
            ...
            if i > len(base_model.layers) - int(0.2*len(base_model.layers)):
...
if not reload_base_ft:
    ...
    print("Model summary")
    print(model.summary())
    print("Fine tuning the MobileNetV3")
    # Train the model in batches 
    idx = 1
    for images, labels in train_dataset:
        print("Training batch {} out of {}".format(idx, len(train_dataset)))
        if idx >9:
            model.fit(x = images, y = labels, epochs=run_params["top_model_epochs"], batch_size = bs, shuffle = True)
        if idx > 14:
            break
        idx +=1
