...
def build_model(input_shape, num_classes=1, trainable_last_layer=False):
    encoder = get_encoder(input_shape, trainable_last_layer)
    classifier = get_classifier(encoder, num_classes)

    model = keras.Model(inputs=encoder.input, outputs=classifier(encoder.output))
...
training_max_batches = 20
use_pooling = False
use_validation_data_in_training = False
use_data_aug = True
...
model = build_model(model_input_shape, num_classes=1, trainable_last_layer=False)
